function components(array) {
    ...
    for (let i = 0; i < array.length; i++) {
        ...
        let containsInSystems = (systemName) => {
            let system = null;
            systems.forEach((el) =>
                el.systemName === systemName ? (system = el) : null
            );
            return system;
        };
        ...
        if (containsInSystems(elements[0]) === null) {
            ...
            let component = system.getComponents.find(
                (el) => el.name === elements[1]
            );
    ...
    for (let system of Object.values(systems)) {
        ...
        for (let component of components.sort(
            (a, b) => b.subcomponents.length - a.subcomponents.length
        )) {
